...
class Batch(DotDict):
    ...
    """
    def __init__(self, inter_times: torch.Tensor, mask: torch.Tensor, marks: Optional[torch.Tensor] = None, target: Optional[torch.Tensor] = None, **kwargs):
    ...
    def _validate_args(self):
        ...
        if self.target is not None:
            if self.target.ndim != 1:
                raise ValueError(f"target must be a 1-d tensor (got {self.target.ndim}-d)")
            if self.target.shape[0] != (self.size):
                raise ValueError(
                    f"target must be of shape (batch_size={self.size}), got {self.target.shape}"
                )
    ...
    def from_list(sequences: List[Sequence]):
        ...
        if sequences[0].marks is not None:
            marks = pad_sequence([seq.marks for seq in sequences if seq.marks is not None], max_len=max_seq_len)
        else:
            marks = None

        if sequences[0].target is not None:
            target = torch.tensor([seq.target for seq in sequences if seq.target is not None], device=device, dtype=dtype)
        else:
            target = None

        return Batch(inter_times, mask, marks, target)
    ...
    def get_sequence(self, idx: int) -> Sequence:
        ...
        return Sequence(inter_times=inter_times, marks=marks, target=target)
    ...
    def to(self, device: Union[str, torch.device]) -> 'Batch':
        ...
        Returns:
            New Batch instance with tensors on the target device
        """
        # Move standard tensors
        inter_times = self.inter_times.to(device)
        mask = self.mask.to(device)
        marks = self.marks.to(device) if self.marks is not None else None
        target = self.target.to(device) if self.target is not None else None
        
         # Get additional attributes (excluding the standard ones)
        additional_attrs = {
            k: v.to(device) if torch.is_tensor(v) else v
            for k, v in self.__dict__.items()
            if k not in {'inter_times', 'mask', 'marks', 'target'}
        }
    
        return Batch(inter_times=inter_times, mask=mask, marks=marks, target=target, **additional_attrs)
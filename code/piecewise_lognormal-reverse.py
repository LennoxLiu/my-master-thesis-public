...
from entropy_tpp import TE_estimation_tpp, run_multiple_estimations
import torch
...
def compute_reference(count_table, intensity_table, total_duration):
    toltal_length = sum(count_table.values())
    # p(Y_t+1=1|Y_t,X_t)
    H_yyx = 0
    for (y_t, x_t) in {(0,0), (0,1), (1,0), (1,1)}:
        frequency = count_table[(y_t, x_t)] / toltal_length
        H_yyx += frequency * lognormal_entropy(*intensity_table[(y_t, x_t)]) if frequency > 0 else 0

    p_y_small = (count_table[(1,0)] + count_table[(1,1)]) / toltal_length
    p_x_small = (count_table[(0,1)] + count_table[(1,1)]) / toltal_length

    # print("\n--- Gauss-Laguerre Entropy Reference Calculation ---")
    # H_y_small = gauss_laguerre_entropy( *intensity_table[(True, True)],*intensity_table[(True, False)], p_x_small)
    # H_y_large = gauss_laguerre_entropy( *intensity_table[(False, True)],*intensity_table[(False, False)], p_x_small)
    # H_yy = p_y_small * H_y_small + (1 - p_y_small) * H_y_large
    # print(f"- [GLQ]Reference H(Y_t+1|Y_t) (nats per event): {H_yy}")
    # print(f"- [GLQ]Reference H(Y_t+1|Y_t) (nats per second): {H_yy* toltal_length / total_duration}")
    # # print(f"- [GLQ]Reference H(Y_t+1|Y_t,X_t) (nats per event): {H_yyx}")
    # # print(f"- [GLQ]Reference H(Y_t+1|Y_t,X_t) (nats per second): {H_yyx* toltal_length / total_duration}")
    # TE = H_yy - H_yyx
    # print(f"- [GLQ]Reference Transfer Entropy (nats per event): {TE}")
    # print(f"- [GLQ]Reference Transfer Entropy (nats per second): {TE * toltal_length / total_duration}")
    ...
    print(f"- [MC]Reference H(Y_t+1|Y_t) (nats per second): {H_yy* toltal_length / total_duration}")
    # print(f"- [MC]Reference H(Y_t+1|Y_t,X_t) (nats per event): {H_yyx}")
    # print(f"- [MC]Reference H(Y_t+1|Y_t,X_t) (nats per second): {H_yyx* toltal_length / total_duration}")
    TE = H_yy - H_yyx
    print(f"- [MC]Reference Transfer Entropy (nats per event): {TE}")
    print(f"- [MC]Reference Transfer Entropy (nats per second): {TE * toltal_length / total_duration}")

    print(f"- Reference H(Y_t+1|Y_t,X_t) (nats per event): {H_yyx}")
    print(f"- Reference H(Y_t+1|Y_t,X_t) (nats per second): {H_yyx* toltal_length / total_duration}")
...
if __name__ == "__main__":
    ...
    configs = {
            ...
            "device": device,
            "num_samples": 20000,  # Number of samples to use for the Monte Carlo sampling in EstimateContinuousTE
            "verbose": False,  # Whether to print the training statistics
        }

    (TE_test, H_yy_test, H_yyx_test), (log_loss_yy, log_loss_yyx), msg1 = TE_estimation_tpp(
            event_time=[torch.tensor(x_events), torch.tensor(y_events)], 
            configs=configs, 
            seed=seed
    )
    ...
    run_multiple_estimations(
        target_events=torch.tensor(x_events),
        source_events=torch.tensor(y_events),
        configs=configs,
        repeat_times=10,
        seed=seed
    )
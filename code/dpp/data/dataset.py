...
def load_dataset(name: str, folder: Union[Path, str] = dataset_dir):
    ...
    def get_inter_times(seq: dict) -> torch.Tensor:
        """Get inter-event times from a sequence."""
        return torch.tensor(np.ediff1d(np.concatenate([[seq["t_start"]], seq["arrival_times"], [seq["t_end"]]])))
...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def get_dataloader(
        ...
        return torch.utils.data.DataLoader(
            self, batch_size=batch_size, shuffle=shuffle, collate_fn=Batch.from_list, pin_memory=True,
    ...
    def get_inter_time_statistics(self) -> Tuple[float, float]:
        ...
        return mean_log_inter_time.item(), std_log_inter_time.item()

    def get_max_target(self) -> float:
        """Get the maximum next inter-event time across all sequences."""
        return max(seq.target.item() for seq in self.sequences)

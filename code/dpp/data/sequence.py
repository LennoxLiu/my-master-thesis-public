...
class Sequence(DotDict):
    ...
    """
    def __init__(self, inter_times: torch.Tensor, marks: Optional[torch.Tensor], target: Optional[torch.Tensor], **kwargs):
        ...
        if target is not None:
            ...
            if target.ndim != 0:
                raise ValueError(f"target must be a 0-d tensor (got {target.ndim}-d)")
            self.target = target
        self._validate_args()

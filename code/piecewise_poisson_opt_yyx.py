...
import optuna
from entropy_tpp import CondH_estimation_yyx, TE_estimation_tpp, run_multiple_estimation, save_dict_indented
import torch
...
def create_objective(arrival_times_target_list, arrival_times_source_list,
    ...
    def objective(trial):
        ...
        n_layers_yyx = trial.suggest_int("n_layers_yyx", 1, 3) # From 1 to 5 hidden layers
        hidden_sizes_yyx = []
        ...
        configs = {
            "model_config_yyx": {
                ...
                "context_size": 2** trial.suggest_int("context_size_yyx", 1, 6),  # From 2^0 to 2^7, i.e., 1 to 128, Size of the RNN hidden vector
                "num_mix_components": 2** trial.suggest_int("num_mix_components_yyx", 1, 7),  # 16 Number of components for a mixture model
            ...
            "train_config_yyx": {
                "L2_weight": trial.suggest_float("L2_weight_yyx", 1e-10, 1e-3, log=True),          # L2 regularization parameter
        ...
        for i in range(len(arrival_times_target_list)):
            ...
            h_yyx_sec, log_loss_yyx = CondH_estimation_yyx(
            ...
            if  h_yyx_sec == float('nan'):
                print(f"Error during TE estimation for run {i+1}. Skipping this run.\n")
                return None, None
        
            print(f'Conditional entropy h_yyx for run {i+1}: {h_yyx_sec:.5f} nats/sec')
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(directions=["minimize"], storage="sqlite:///db.sqlite3"
                                ,load_if_exists=True, study_name=f"poisson_Hyyx_{seed:02d}") # Set direction to 'maximize' for TE,  
